...
def load_sources():
    ...
    with get_conn() as conn:
        for s in sources:
            ...
            cur = conn.execute(
                """INSERT INTO sources (name, base_url, feed_url, default_region, sports_keyword, exclude_keywords, team_routing, local_keyword, status)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')
                   ON CONFLICT(feed_url) DO UPDATE SET
                     name=excluded.name, base_url=excluded.base_url,
                     default_region=excluded.default_region,
                     sports_keyword=excluded.sports_keyword,
                     exclude_keywords=excluded.exclude_keywords, team_routing=excluded.team_routing,
                     local_keyword=excluded.local_keyword,
                     status='active'
                   """,
                (s["name"], s.get("base_url"), s["feed_url"], s["default_region"],
                 s.get("sports_keyword"),
                 "|".join(s["exclude_keywords"]) if s.get("exclude_keywords") else None,
                 "true" if s.get("team_routing") else None,
                 "|".join(s["local_keyword"]) if s.get("local_keyword") else None),
            )
