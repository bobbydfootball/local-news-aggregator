...
def load_sources():
    ...
    with get_conn() as conn:
        for s in sources:
            ...
            cur = conn.execute(
                """INSERT INTO sources (name, base_url, feed_url, default_region, sports_scope, sports_keyword, sports_keyword_scope, exclude_keywords, team_routing, local_keyword, status)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
